...
class IceqMainWindow(QtWidgets.QMainWindow):
    ...
    def update_status_panel(self):
        ...
        # ------------------------------------------------------------
        # LEDs do rodapé + manutenção: spindle e coolant (fonte INTERNA)
        # (não depende do stat, então fica confiável para teste)
        # ------------------------------------------------------------
        try:
            spindle_on = self._spindle_running
            spindle_dir = self._spindle_dir
            ...
            # LEDs da aba manutenção (spindle)
            ...
            if hasattr(self, "led_maint_sig_spindle_stop"):
                # "stop" aceso quando NÃO está girando
                self.set_led(self.led_maint_sig_spindle_stop, not spindle_on)

            # LED da aba manutenção (coolant), se existir
        ...
        # ------------------------------------------------------------
        # ESTADOS (coluna "ESTADO" na aba MANUT) - LÊ PINO HAL REAL
        # ------------------------------------------------------------
        try:
            # Lê pinos HALUI (como aparece na sua tabela do MANUT)
            # Se não conseguir ler (None), cai no fallback (stat/setpoint)
            # Observação: aqui NÃO mexe em LED, só no texto TRUE/FALSE.

            # Fallbacks coerentes (caso o pin não exista)
            try:
                rpm_sp = int(abs(getattr(self, "_spindle_rpm_setpoint", 0)))
            except Exception:
                rpm_sp = 0
            spindle_dir_fb = int(getattr(self, "_spindle_dir", 0))  # 1 / -1 / 0
            spindle_on_fb = (spindle_dir_fb != 0 and rpm_sp > 0)

            # CW / CCW / STOP / COOLANT via HAL
            v_cw   = self._hal_bit("halui.spindle.forward")
            v_ccw  = self._hal_bit("halui.spindle.reverse")
            v_stop = self._hal_bit("halui.spindle.stop")
            v_col  = self._hal_bit("halui.coolant.mist")

            # Se algum retornar None, usa fallback
            if v_cw is None:
            ...
            if v_stop is None:
                # “STOP” como estado lógico: verdadeiro quando NÃO está girando
                v_stop = (not spindle_on_fb)

            if v_col is None:
                # você já calcula coolant_on acima; tenta reaproveitar se existir no escopo
                try:
                    v_col = bool(coolant_on)
                except Exception:
                    v_col = False

            # Atualiza as QLabel *_state (pelos nomes que aparecem no seu print)
            ...
            print(f"[ICEQ] estados MANUT (HAL pins) erro: {e}")

        # ----- RPM do spindle (rodapé) -----
        try:
            rpm_base = self._get_spindle_rpm_safe()

            # Override atual do spindle (0..120). Preferência:
            # 1) variável interna (atualizada pelos handlers do slider/spin)
            # 2) spinbox na tela
            # 3) fallback 100
            ovr = 100
            try:
            ...
            if hasattr(self, "spn_vel_spindle_oper"):
                try:
                    ovr = int(self.spn_vel_spindle_oper.value())
                except Exception:
                    pass

            # clamp 0..120 (mantém seu padrão)
            if ovr < 0:
                ovr = 0
            if ovr > 120:
                ovr = 120

            if rpm_base is not None:
                rpm_base = abs(float(rpm_base))
                rpm_eff = rpm_base * (float(ovr) / 100.0)

                # Mostra o efetivo (com override)
    ...
    def _get_spindle_rpm_safe(self):
        """Tenta obter RPM do spindle via stat; retorna None se não existir."""
        try:
            # Em várias configs: stat.spindle[0]['speed'] ou stat.spindle[0].speed
            sp = getattr(self.stat, "spindle", None)
            if sp is None:
                return None

            s0 = sp[0]
            if isinstance(s0, dict) and "speed" in s0:
                return float(s0["speed"])
